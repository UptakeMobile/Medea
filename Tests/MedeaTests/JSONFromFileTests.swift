...
class JSONFromFileTests: XCTestCase {
  ...
  func testFileWithoutExtension() {
    print("!!!\(fetchFakeBundle())")
    let json = try? JSONHelper.anyJSON(fromFileNamed: "object", bundle: fetchFakeBundle())
    XCTAssertNotNil(json)
  }
  
  
  func testFileWithExtension() {
    let json = try? JSONHelper.anyJSON(fromFileNamed: "object", extension: "ext", bundle: fetchFakeBundle())
    XCTAssertNotNil(json)
  }
  
  
  func testObjectFile() {
    let json = try! JSONHelper.jsonObject(fromFileNamed: "object", bundle: fetchFakeBundle())
    XCTAssertEqual(json["hello"] as! String, "world")
  }
  
  
  func testArrayFile() {
    let json = try! JSONHelper.jsonArray(fromFileNamed: "array", bundle: fetchFakeBundle())
